...
public class AZSideBarView extends View {
	...
	private void initAttribute(AttributeSet attrs, int defStyleAttr) {
		...
		mBarWidth = typeArray.getDimensionPixelOffset(R.styleable.AZSideBarView_barWidth,
													  (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 0,
																					  getResources().getDisplayMetrics()));
		mBarHeight = typeArray.getDimensionPixelOffset(R.styleable.AZSideBarView_barheight,
				(int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 0,
						getResources().getDisplayMetrics()));
		if (mBarWidth == 0) {
			mBarWidth = getWidth();
			Log.e("mbarwidt",mBarWidth + ".");
		}
		if (mBarHeight == 0) {
			mBarHeight = 3 * mTextSize;
			Log.e("heightbar",mBarHeight + ".");
		}
		typeArray.recycle();
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		Log.e("contentLi",getMeasuredWidth() + "." + mBarWidth + "." + mBarPadding);
		float contentLeft = getMeasuredWidth() - mBarWidth - mBarPadding;
		float contentRight = getMeasuredWidth() - mBarPadding * 3;
...
//		mSlideBarRect.set(contentLeft, contentTop, contentRight, contentBottom);
		mSlideBarRect.set(contentTop * 3, contentTop, contentRight, contentTop * 10);
	}
	...
	private void drawLetters(Canvas canvas) {
		...
		canvas.drawRoundRect(mSlideBarRect, mBarHeight / 2.0f, mBarHeight / 2.0f, mPaint);
		//绘制描边
		mPaint.setStyle(Paint.Style.STROKE);
		mPaint.setColor(mStrokeColor);
		canvas.drawRoundRect(mSlideBarRect, mBarHeight / 2.0f, mBarHeight / 2.0f, mPaint);
		//顺序绘制文字
		float itemHeight = (mSlideBarRect.right - mSlideBarRect.left) / mLetters.size();
		for (int index = 0; index < mLetters.size(); index++) {
			...
			float pointY = mSlideBarRect.top / 2.0f + (mSlideBarRect.bottom - mSlideBarRect.top) / 2.0f - mTextPaint.ascent() / 2.0f ;
			canvas.drawText(mLetters.get(index),  baseLine, pointY,mTextPaint);
		}
	}
	...
	private void drawSelect(Canvas canvas) {
		if (mSelect != -1) {
			...
			float pointY = mSlideBarRect.top / 2.0f + (mSlideBarRect.bottom - mSlideBarRect.top) / 2.0f - mTextPaint.ascent() / 2.0f ;
			canvas.drawText(mLetters.get(mSelect),  baseLine, pointY,mTextPaint);
		}
	}
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		mPreSelect = mSelect;
		mNewSelect = (int) (x / (mSlideBarRect.right - mSlideBarRect.left) * mLetters.size());
		switch (event.getAction()) {
			case MotionEvent.ACTION_DOWN:
//				Log.e("downnn",x + ":" + y + ":" + mSlideBarRect.left + ":" + mSlideBarRect.top + ":" + mSlideBarRect.bottom);
